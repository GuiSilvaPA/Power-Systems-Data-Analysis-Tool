...
def patch_file(file_path: str, content: bytes, extra: dict = None) -> bytes:
    ...
    if file_path == 'index.html':
        ...
        extra_jsons = f'''var patched_jsons_content={{{','.join(["'/" + k + "':" + v.decode("utf8") + "" for k, v in extra.items()])}}};'''
        others      = f'''<footer> <script>''' + extra_jsons + '''const origFetch = window.fetch;
        ...
        patched_content = index_html_content.replace('<footer>', others).replace('href="/', 'href="').replace('src="/', 'src="')
